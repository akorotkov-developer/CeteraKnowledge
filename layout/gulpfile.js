...
let images = () => gulp.src(paths.src.images, {
    allowEmpty: true
  })
...
let imagesMinify = () => gulp.src(paths.src.images, {
    allowEmpty: true
  })
  .pipe(gulpNewer(paths.dist.images))
  .pipe(tinypng({
...
let i = () => gulp.src(paths.src.i, {
  allowEmpty: true
}).pipe(gulp.dest(paths.dist.i));

let iMinify = () => gulp.src(paths.src.i, {
  allowEmpty: true
}).pipe(tinypng({
...
let jsLib = () => gulp.src(paths.src.jsLib, {
    allowEmpty: true
  })
  .pipe(gulpUglify())
...
let jsApp = () => gulp.src(paths.src.js, {
    allowEmpty: true
  })
  .pipe(gulpBabel({
    presets: ['@babel/env']
...
let json = () => gulp.src(paths.src.json, {
    allowEmpty: true
  })
  .pipe(gulp.dest(paths.dist.json));

let jsAppMinify = () => gulp.src(paths.src.js, {
    allowEmpty: true
  })
  .pipe(gulpBabel({
    presets: ['@babel/env']
...
let reloadBrowser = () => gulp.src(paths.src.js, {
    allowEmpty: true
  }).pipe(gulpWait(50))
  .pipe(gulpBrowserSync.reload({
    stream: true
